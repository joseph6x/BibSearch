...
//        return parser.text('every 24 hours');
    ...
    */
    ...
    SyncedCron.add({
      ...
      schedule: function(parser) {
        // parser is a later.parse object
        return parser.text('every 360 hours');
      },
      job: function() {
     ...
     //return "Esto es un string";
        ...
        for (var i = 0; i < endp.length; i++) {
                              ...
                              //  }
                                ...
                                for (var t = 0; t < k.length; t++) {
                                    ...
                                    if (k[t].firstResult) {
                                        ...
                                        var FacetedResum_Sources2 = FacetedResum_Sources.filter(function (d) {
                                            return d.key == FA[4].value;
                                        });
                                        if (FacetedResum_Sources2.length > 0) {
                                            var indx = FacetedResum_Sources.indexOf(FacetedResum_Sources2[0]);
                                            FacetedResum_Sources[indx].count = FacetedResum_Sources[indx].count + 1;
                                        } else {
                                            FacetedResum_Sources.push({key: FA[4].value, count: 1});
                                        }
                                ...
                                FacetedResum_Sources.sort(function (a, b) {
                                    if (a.count < b.count)
                                        return 1;
                                    if (a.count > b.count)
                                        return -1;
                                ...
                                h.facetedTotalsN = {Years: FacetedResum_Years, Endpoints: FacetedResum_Endpoints, Langs: FacetedResum_Langs, Types: FacetedResum_Types, Sources: FacetedResum_Sources};
                     ...
                     //   } else {
                      ...
                      //  }
                        ...
                        if (0 == k.length) {
                            ...
                            for (var t = 0; t < q; t++) {
                              ...
                              //  }
                                ...
                                //Source of resource
                                if (m.results.bindings[t].organization_des != undefined) {
                                    fSource = m.results.bindings[t].organization_des.value;
                                }
                                //    r_Year["" + v] = fYear;
                                // }
                                ...
                                bulk.push({
                                   ...
                                   // score: Number(fScore),
                                    uri: v,
                                   // sub: fSub,
                                    faceted: [{key: 'Year', value: Number(fYear) == 0 || isNaN(Number(fYear)) ? null : Number(fYear)}, {key: 'Endpoint', value: fEndpoint}, {key: 'Lang', value: fLang}, {key: 'Type', value: fType},{key: 'Sources', value: fSource}],
                                    firstResult: un,
                                    original: orgi
                                });
                                /*Cache.insert({
                        ...
                        if (cacheo) {
                            ...
                            var sources = Cache.distinct('faceted.4.value', {key: j, firstResult: true, faceted: {$elemMatch: {key: 'Sources'}}});
                            var sources2 = [];
                            for (var s = 0; s < sources.length; s++) {
                                var re_co = Cache.find({key: j, firstResult: true, faceted: {$elemMatch: {key: 'Sources', value: sources[s]}}}).count();
                                sources2.push({key: sources[s], count: re_co});
                            }
                            ...
                            sources2 = sources2.filter(function (d) {
                                return d.count > 0;
                            });
                            var k2_up = Cache.update({key: j, original: true}, {$set: {facetedTotals: {Years: years2, Endpoints: endpoints2, Langs: langs2, Types: types2, Sources: sources2}}});
                        }
            ...
            runQuery: function (endpointURI, defaultGraph, query, format, timeout, typeServer) {
                ...
                timeout = _.isUndefined(timeout) ? '10000' : timeout;
